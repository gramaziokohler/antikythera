...
class Orchestrator:
    ...
    def update_blueprint_structure(self, blueprint_id: str, updated_tasks: list[dict]) -> None:
        """Updates the structure (dependencies) of a blueprint.

        Can only be called when the orchestrator is PAUSED.
        Only allows updating tasks that are PENDING.
        ...
        updated_tasks : list[dict]
            A list of dictionary representations of the tasks with their NEW dependencies.
            Each dictionary MUST include at least the "id" and "depends_on" fields.
        """
        if self._state not in (OrchestratorState.PAUSED, OrchestratorState.IDLE):
            raise RuntimeError("Orchestrator must be PAUSED or IDLE to update blueprint structure.")

        # Find the blueprint
        blueprint = None
        if self.session.blueprint.id == blueprint_id:
            blueprint = self.session.blueprint
        else:
            blueprint = self.session.inner_blueprints.get(blueprint_id)

        if not blueprint:
            raise ValueError(f"Blueprint with ID {blueprint_id} not found.")

        # Validate that all referenced dependency IDs exist in the blueprint
        # We collect all task IDs first
        all_task_ids = {t.id for t in blueprint.tasks}

        # Apply updates
        for task_update in updated_tasks:
            task_id = task_update.get("id")
            if not task_id:
                LOG.warning("Received task update without ID. Skipping.")
                continue

            # Find the original task object
            original_task = next((t for t in blueprint.tasks if t.id == task_id), None)
            if not original_task:
                raise ValueError(f"Task {task_id} not found in blueprint {blueprint_id}.")

            # Task State Validation
            if original_task.state != TaskState.PENDING:
                raise ValueError(f"Cannot update task {original_task.id} because it is in state {original_task.state}. Only PENDING tasks can be updated/reordered.")

            # Update dependencies
            new_dependencies_data = task_update.get("depends_on", [])
            new_dependencies = []

            for dep_data in new_dependencies_data:
                # dep_data can be dict or Dependency object
                dep_id = None
                if isinstance(dep_data, dict):
                    ...
                    new_dependencies.append(Dependency(**dep_data))
                elif isinstance(dep_data, Dependency):
                    dep_id = dep_data.id
                    if dep_id and dep_id not in all_task_ids:
                        raise ValueError(f"Task {task_id} depends on unknown task {dep_id}")
                    new_dependencies.append(dep_data)
                else:
                    LOG.warning(f"Invalid dependency format for task {task_id}: {dep_data}")

            original_task.depends_on = new_dependencies

        # Rebuild graph
        self._build_graph()

        # Update Scheduler with new graph
        self.scheduler.graph = self.graph

        # Persist the updated blueprint
        self.session_storage.store_blueprint(blueprint)
        self.session_storage.update_session_blueprint_state(blueprint)

        LOG.info(f"Updated structure for blueprint {blueprint_id}")
