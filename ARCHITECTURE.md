...
* `Valid Blueprint`: A blueprint the orchestrator can run to completion. Beyond being well-formed — exactly one `START` and one `END`, no orphan tasks, every dependency naming a task that exists — it must be acyclic, `END` must be its only sink, and every dependency must declare an implemented `DependencyType`. Validity is checked when the blueprint is constructed, so an invalid blueprint never reaches a session. See [ADR-0005](docs/adr/0005-blueprint-validation-rejects-unrunnable-graphs.md).
* `Dependency Type`: How a `Task` waits on a task it depends on. `FS` (Finish-to-Start, the default) waits for the predecessor to finish; `SS` (Start-to-Start) waits only for it to start running; `FF` (Finish-to-Finish) allows the task to start but not to finish before the predecessor finishes. A `SKIPPED` predecessor satisfies all three. Start-to-Finish is not supported.
