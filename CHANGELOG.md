...
- Property-based tests over generated blueprints, covering blueprint validation and the task scheduler. A shared hypothesis generator (`tests/support/blueprint_strategies.py`) emits valid blueprints built over a fixed topological order, and a deterministic in-process run simulation (`tests/support/simulation.py`) drives the real `TaskScheduler` with no broker, storage or threads involved. The properties assert that a run always terminates, that completion order does not change the final state, that a duplicate or late completion cannot change a task that already finished, that no task is dispatched before its dependencies permit it, and that any task left non-terminal has a failed ancestor. `hypothesis` is added as a development dependency. See ADR-0006.
...
* **Breaking:** `Blueprint.validate()` now rejects blueprints the orchestrator cannot run, not just malformed ones. A dependency cycle is rejected, naming the tasks that form it; a task with no path to `END` is rejected, naming the tasks that cannot reach it. Both used to load cleanly and then fail silently at runtime — a cycle stalls the session forever with no error, and a second sink makes session completion depend on graph iteration order. All 18 blueprints in `examples/` are unaffected. See ADR-0005.
* **Breaking:** `Dependency` now coerces its `type` through `DependencyType`, raising on any unrecognised value instead of storing it verbatim. An unrecognised type used to match no branch in the scheduler's readiness check, leaving an empty precondition set that is vacuously satisfied, so the task dispatched immediately with its dependency ignored entirely. `SF` (Start-to-Finish) is removed from `DependencyType` and from the published blueprint JSON schema; it was implemented in neither the readiness check nor the queue, no example blueprint uses it, and it does not appear in the protobuf schema. See ADR-0005.
