...
def test_google_only_template_does_not_require_github_credentials(tmp_path):
    oauth_secret = tmp_path / "oauth"
    google_secret = tmp_path / "google"
    oauth_secret.write_text("shared-secret")
    google_secret.write_text("google-secret")

    template = (REPOSITORY_ROOT / "config/auth/dex.google.yaml").read_text()
    rendered = render_dex_config(
        template,
        {
            "AUTH_PUBLIC_URL": "http://localhost",
            "OAUTH2_PROXY_CLIENT_SECRET_FILE": str(oauth_secret),
            "GOOGLE_CLIENT_ID": "google-id",
            "GOOGLE_CLIENT_SECRET_FILE": str(google_secret),
        },
    )

    assert "type: google" in rendered
    assert "google-secret" in rendered
    assert "type: github" not in rendered.lower()


def test_default_template_does_not_require_commented_future_providers(tmp_path):
    for name in ("oauth", "google", "github"):
        (tmp_path / name).write_text(f"{name}-secret")

    template = (REPOSITORY_ROOT / "config/auth/dex.yaml").read_text()
    rendered = render_dex_config(
        template,
        {
            "AUTH_PUBLIC_URL": "http://localhost",
    ...
    assert "type: google" in rendered
    assert "type: github" in rendered
    assert "${APPLE_CLIENT_ID}" not in rendered
    assert "${EDUID_CLIENT_ID}" not in rendered
...
@pytest.mark.skipif(shutil.which("docker") is None, reason="Docker Compose CLI is not installed")
def test_google_only_compose_profile_removes_github_secret():
    result = subprocess.run(
        [
            "docker",
            "compose",
            ...
            "-f",
            "docker-compose.auth.google.yml",
            "config",
            "--format",
            "json",
        ],
    ...
    dex_init_secrets = {secret["source"] for secret in services["dex-init"]["secrets"]}
    assert dex_init_secrets == {"oauth2_proxy_client_secret", "google_client_secret"}
    assert services["dex-init"]["volumes"][0]["source"].endswith("config/auth/dex.google.yaml")
